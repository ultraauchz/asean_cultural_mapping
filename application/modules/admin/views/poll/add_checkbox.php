<div class="form-group <?php echo $uid?>">
    <label class="col-sm-2 control-label">หัวข้อคำถาม<br /><small>(checkbox)</small></label>
    <div class="col-lg-4">
        <div class="input-group">
            <input type="hidden" name="question_title_reference_id[]" value="<?php echo $uid?>" />
            <input type="hidden" name="question_type_<?php echo $uid?>" value="3" />
	...
	<?php
		if(count($rs->survey_question_choice->all) == 0) {
			echo modules::run('admin/poll/add_checkbox_choice', $uid);
		} else {
			foreach($rs->survey_question_choice as $item) {
				echo modules::run('admin/poll/add_checkbox_choice', $uid, $item->id);
			}
		}
	?>
	...
	<div>
	    ...
	    <div class="col-lg-4">
	    	<button type="button" onclick="addChoice('checkbox','<?php echo $uid?>')" ><span class="glyphicon glyphicon-plus" ></span> เพิ่มตัวเลือก</button>
	    </div>
    </div>
