    ...
    <meta charset="utf-8">
		...
		<?php
			...
			if($rs->all) {
				...
				$ebook->set_path(site_url().'js/turnjs4');
				//$ebook->set_size($attr_book['size']['width'], $attr_book['size']['height']);
				
				foreach($rs as $item) {
					$ebook->set_page(site_url().'uploads/ebook/'.$item->tmp_name);
				}
			}
		...
		<script language='javascript'>
			$(function(){
				...
				$('.print_page').on('click', function(){
					id = '<? echo $book->id; ?>';
					href = '<? echo site_url(); ?>ebooks/print_page/'+id+'/'+$('#position_page_'+$(this).attr('direction')).val();
...
<body>
		...
		if(count($rs->all) != 0) { ?>
			...
			<div style='display:inline-block; min-width:100%; text-align:center; background:#ccc; border-radius:4px;' id="wrap_book">
				...
				<div style='padding:20px 0px; background:#fff; border-top:solid 3px #08c;' class='hide_print' id='book_header'>
					<h2> ตัวอย่าง หนังสือ : <?php echo $book->title; ?></h2>
		
					<button id='zoom-in2' direction='zoomin' class='view_page btn btn-info'><span class='glyphicon glyphicon-plus'></span></button>
					<button id='zoom-out2' direction='zoomout' class='view_page btn btn-info' style='margin-right:20px;'><span class='glyphicon glyphicon-minus'></span></button>  
					
					<button direction='prev' class='change_page btn btn-info'><span class='glyphicon glyphicon-chevron-left'></span></button>
					...
					<button id='go2' direction='go' class="change_page btn btn-info"><span class='glyphicon glyphicon-search'></span> ไปที่หน้า</button> 
					
					<?php echo '<span class="text_currentPage">1</span> / '.count($rs->all); ?> Pages
		
					<button direction='next' class='change_page btn btn-info'><span class='glyphicon glyphicon-chevron-right'></span></button>
				</div>
